...
app.put("/api/reports/:id", (req, res) => {
    ...
    const sql = "UPDATE reports SET report_name = ?, report_date = ?, details = ?, employee_id = ? WHERE id = ?";
    db.query(sql, [report_name, report_date, details, employee_id, id], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
